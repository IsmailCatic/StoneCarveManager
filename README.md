...
3. Wait until all services (WebAPI, SQL Server, RabbitMQ, Email Service) start successfully. ⏳
...
1. Extract the archive containing the desktop build.

2. Inside the `Release` folder, run:
   ```
   stonecarve_manager_desktop.exe
   ```

3. Log in using the admin or employee credentials (see below).
...
1. Extract the archive containing the mobile build.

2. Inside the `flutter-apk` folder, locate:
   ```
   app-release.apk
   ```
   Drag it onto the emulator or install it on a physical device.

3. Launch the application and log in using the test credentials (see below).
